  ...
  return (
    <>
        ...
        <div className="relative mx-auto max-w-5xl px-6 text-center">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-4 py-1.5">
            ...
            <span className="text-xs font-medium text-indigo-300">
              AI Automation for Plumbing & HVAC
            </span>
          </div>

          <h1 className="text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-6xl leading-tight">
            Stop Losing Jobs to<br className="hidden sm:block" />
            <span className="text-indigo-400"> Missed Calls & Cold Estimates</span>
          </h1>
          <p className="mt-6 text-lg text-slate-300 leading-relaxed max-w-2xl mx-auto sm:text-xl">
            KraftAI answers every lead in under 30 seconds, books appointments automatically, and follows up on every estimate — so you close more jobs without hiring more office staff.
          </p>
          ...
          <div className="mt-12 flex flex-wrap items-center justify-center gap-8 text-slate-400 text-sm">
            <div className="flex items-center gap-2">
              <Wrench className="h-4 w-4 text-emerald-400" aria-hidden="true" />
              <span>Works with ServiceTitan & Housecall Pro</span>
            </div>
            <div className="flex items-center gap-2">
        ...
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mt-12 text-center">
            <p className="text-lg text-slate-400">
              You&apos;re not alone — <span className="text-white font-semibold">60% of plumbing estimates</span> go unanswered because nobody follows up.
            </p>
        ...
        <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="grid gap-8 sm:grid-cols-2">
            {niche.useCases.map((uc, idx) => {
              const Icon = useCaseIcons[uc.icon] || PhoneCall;
              return (
      ...
      <section className="bg-slate-950 py-20 sm:py-24 border-t border-slate-800/50" aria-labelledby="blog-preview-heading">
        <div className="mx-auto max-w-6xl px-6">
          <h2 id="blog-preview-heading" className="text-3xl font-bold text-white text-center sm:text-4xl">
            Latest Insights for Home Service Pros
          </h2>
      ...
      {niche.cities && niche.cities.length > 0 && (
        <section className="bg-slate-900 py-20 sm:py-24" aria-labelledby="cities-heading">
          <div className="mx-auto max-w-6xl px-6">
            <h2 id="cities-heading" className="text-3xl font-bold text-white text-center sm:text-4xl">
              Serving Contractors Across the US
            </h2>
            <p className="text-center text-slate-400 mt-4 mb-12 max-w-2xl mx-auto">
              We work with plumbing and HVAC companies in major metros. Find your city below.
            </p>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
              {niche.cities.map((city) => (
                <Link key={city.slug} href={`/${city.slug}`}
                  className="rounded-xl border border-slate-800 bg-slate-950 p-4 text-center transition-all hover:border-indigo-500/30 hover:shadow-lg hover:shadow-indigo-500/5">
                  <p className="text-sm font-semibold text-white">{city.name}, {city.state}</p>
                  <p className="text-xs text-slate-400 mt-1">{city.population} population</p>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="mx-auto max-w-4xl px-6">
          <div className="text-center mb-12">
            ...
            <p className="mt-4 text-lg text-slate-400 max-w-xl mx-auto">
              Get a free automation audit — we&apos;ll show you exactly where leads are slipping through and how to fix it in 48 hours.
            </p>
          </div>
      ...
      <ExitIntentPopup source="homeservices-exit-intent" />
      <StickyMobileCTA ctaText="Get Your Free Audit" urgencyText="Only 3 founding-client spots left for home services" />
      <SocialProofToast messages={[
        { city: 'Houston, TX', action: 'booked an automation audit', timeAgo: '8 minutes ago' },
        { city: 'Atlanta, GA', action: 'requested a missed-call demo', timeAgo: '22 minutes ago' },
        { city: 'Nashville, TN', action: 'signed up for founding pricing', timeAgo: '1 hour ago' },
        { city: 'San Diego, CA', action: 'booked a scheduling demo', timeAgo: '2 hours ago' },
        { city: 'Dallas, TX', action: 'requested after-hours call setup', timeAgo: '4 hours ago' },
      ]} />
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbSchema([
        ...
        { name: 'Home Services', url: BASE_URL },
      ])) }} />
    </>
  );
}
