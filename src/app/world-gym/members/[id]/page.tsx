...
import { ArrowLeft, Save, MessageCircle, User, Phone, MapPin, Calendar, DollarSign, Hash, CheckCircle, Loader2, Pencil } from 'lucide-react';
...
export default function EditMemberPage() {
  ...
  useEffect(() => {
    const fetchMember = async () => {
      try {
        const res = await fetch(`/world-gym/api/members?id=${memberId}`);
        ...
        setFormData({
          ...
          district: member.district || 'Bulandshahr',
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Edit Member</h1>
          <p className="text-sm text-slate-400">Update member details</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {success && (
          <div className="flex items-center gap-3 p-4 bg-green-500/20 border border-green-500/30 rounded-xl">
            ...
            <p className="text-green-400 font-medium">Member updated successfully! Redirecting...</p>
          </div>
        )}
        ...
        <div className="flex gap-4">
          ...
          {formData.phone && (
            <a 
              href={`https://wa.me/91${formData.phone}?text=Hello ${formData.name}, Your membership details have been updated at World Gym.`}
              target="_blank"
